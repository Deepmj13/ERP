...
export async function cleanupTenant(
  ...
  const steps = [
    ...
    () => prisma.vendorPaymentAllocation.deleteMany({ where: { tenantId } }),
    () => prisma.vendorPayment.deleteMany({ where: { tenantId } }),
    () => prisma.vendorBillItem.deleteMany({ where: { tenantId } }),
    () => prisma.vendorBill.deleteMany({ where: { tenantId } }),
    () => prisma.goodsReceiptItem.deleteMany({ where: { tenantId } }),
    () => prisma.goodsReceipt.deleteMany({ where: { tenantId } }),
    () => prisma.purchaseOrderItem.deleteMany({ where: { tenantId } }),
    () => prisma.purchaseOrder.deleteMany({ where: { tenantId } }),
    () => prisma.purchaseRequestItem.deleteMany({ where: { tenantId } }),
    () => prisma.purchaseRequest.deleteMany({ where: { tenantId } }),
