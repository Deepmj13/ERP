---
name: erp-saas
description: Use when working on the ERP_SAAS multi-tenant SaaS ERP codebase (NestJS + Prisma + PostgreSQL 16 + RLS + Redis/BullMQ + Flutter monorepo). Covers implementation status, architecture rules, the NestJS module pattern, database + API conventions, document numbering, idempotency, RBAC, ADRs, and how to add/extend ERP modules (Sales, Inventory, Finance, Procurement, HR, Payroll, Operations, SaaS platform). Trigger on files under apps/api, apps/worker, packages/, database/prisma, or when building a new ERP module, migration, permission code, or e2e spec.
---
...
- Phase 3 **Sales** — backend implemented (quotations, orders, deliveries, invoices, payments, bank-accounts). `sales-flow.e2e-spec.ts` still red (invoice-post 500, payment-capture 400). Flutter G-5 not started.
- Phase 4-9 **NOT STARTED** (Inventory, Finance, Procurement, HR/Payroll, Operations, SaaS).
- Cross-cutting gaps: G-1/G-2/G-3/G-6 done; **G-7 e2e green in progress; G-4 seeds, G-5 Flutter scaffold, G-8 permission catalog pending.**
...
apps/api/test        Jest e2e suite (e2e-helpers.ts + 4 specs)
...
4. **Complex SQL must be raw + parameterized** (`$queryRaw`/`$executeRaw`): CTEs, window functions, recursive walks, `FOR UPDATE` locks (stock on hand, sequence allocation, trial balance, account rollups). Raw queries are documented under `apps/api/sql/` and covered by an integration test. Never string-concatenate SQL.
...
8. Keep raw SQL parameterized, documented in `apps/api/sql/`, and integration-tested.
...
8. Update `future.md` only when a corresponding item is fully implemented and verified.
...
10. Before declaring work complete, verify that the implementation matches the source-of-truth documents and existing architecture.
...
[ ] Add audit logging
[ ] Update future.md only after verification
```
